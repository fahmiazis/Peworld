...
// import action
import seekerAction from '../redux/actions/seeker';
...
const HomeSeeker = () => {
  ...
  const company = useSelector((state) => state.seeker);
  const {token} = auth;
  ...
  const {profileCompany, listCompany} = company;
  const {profileJobSeeker} = seeker;
  useEffect(() => {
    SplashScreen.hide();
    dispatch(SeekerAction.getListOfCompany(token));
